...
# Function to check if all images have their label files
            ...
            try:
                ...
                for line_num, line in enumerate(lines, 1):
                    ...
                    try:
                        ...
                        # Checking normalization (0-1 range)
                        ...
                        # Checking if the bounding box is within image boundaries
...
def delete_large_images(image_dir, sizes, min_size=820):
    ...
    print(f"\n Found {len(large_images)} large images (>{min_size}px)")
...
def check_image_sizes(image_dir, target_width=640, target_height=640, ask_delete=False):
    ...
    print(f"--> Matching target size: {len(target_size_images)}")
    print(f"--> Perfect range (416-640): {len(perfect_range_images)}")
...
def print_pixel_range_report(results: Dict[str, List[str]]) -> None:
    ...
    if not results["invalid_range"] and not results["unreadable"]:
        print("--> All images are in standard [0, 255] range")
...
#Calling Function to check for duplicate filenames in the directory below
DATASET_DIRECTORY = "../dataset"
find_duplicate_filenames(DATASET_DIRECTORY)

#Calling Function to check if all images in the following directory are readable
check_images("../dataset/test/images")

#Calling Function to check if all images have their corresponding label
check_labels("../dataset/test/images", "../dataset/test/labels")

#Calling Function to check if all labels in the following directory are in YOLO format
validate_annotations("../dataset/test/labels")

#Calling Function to check if all images in the following directory are in the expected size
check_image_sizes("../dataset/test/images", ask_delete=True)

#Calling Function to check if all images in the following directory are in the expected pixel range -- Normalization check
target_dir = "../dataset/test/images"
res = check_pixel_range(target_dir)
print_pixel_range_report(res)

#Calling Function to check if all images in the following directory are of good quality and remove low-quality pictures
